class DevanadoManagement::DevanadoTemplatesController < ApplicationController
  ...
  def create
    @devanado_template = DevanadoTemplate.new(model_params)
    if @devanado_template.save
      flash[:notice] = 'Se guardaron los datos del Template.'
      flash[:type_message] = 'success'
      redirect_to electrical_management_transformer_electricals_path(params[:transformer_id])
    else
      flash[:notice] = 'Error al guardar los datos del Template, ya hay 1 registro con el mismo nombre.'
      flash[:type_message] = 'danger'
      redirect_to new_devanado_management_transformer_devanado_template_path(@devanado_template.transformer_id)
    end 
  end
  ...
  def update
    if @devanado_template.update(model_params)
      flash[:notice] = 'Se guardaron los datos del Template.'
      flash[:type_message] = 'success'
      redirect_to electrical_management_transformer_electricals_path(params[:transformer_id])
    else
      flash[:notice] = 'Error al guardar los datos del Template, ya hay 1 registro con la misma fecha.'
      flash[:type_message] = 'danger'
      redirect_to new_devanado_management_transformer_devanado_template_path(@devanado_template.transformer_id)
    end 
  end
  ...
  private
    ...
    # Use callbacks to share common setup or constraints between actions.
    def set_model
      @devanado_template = DevanadoTemplate.find(params[:id] )
    end
    
    # Use callbacks to share common setup or constraints between actions.
    def set_model_new
      @devanado_template = DevanadoTemplate.new
      3.times do
        @nested_contact = @devanado_template.devanado_template_details.build
      end       
    end

    # Never trust parameters from the scary internet, only allow the white list through.
    def model_params
       params.require(:devanado_template).permit!
    end
end