class DuvalManagement::PentagonGraphsController < ApplicationController
  ...
  private
    ...
    def list_chromatographicals
       @list_chromatographicals = Chromatographical.where(transformer_id: @transformer.id, deleted: 0).order('date_rehearsal ASC')
    end
    ...
    def display_duval_text
       @duval_type1 = Duval.where(duval_type_id: 2, graph_type: 1)
       @duval_type2 = Duval.where(duval_type_id: 2, graph_type: 2)
       @duval_type3 = Duval.where(duval_type_id: 2, graph_type: 3)
    end
