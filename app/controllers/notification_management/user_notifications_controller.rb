class NotificationManagement::UserNotificationsController < ApplicationController
  ...
  def index
    if user_permission.include?(123)
      @user_notifications = UserNotification.where("deleted = 0 AND state= 0 AND user_id = ? ",current_user.id)
  ...
  def new
    if user_permission.include?(124)
  ...
  def destroy
    if user_permission.include?(127)
      @user_notification.update_attribute(:deleted,1)
