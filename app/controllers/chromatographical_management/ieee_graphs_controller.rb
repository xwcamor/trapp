class ChromatographicalManagement::IeeeGraphsController < ApplicationController
  ...
  def index
    if user_permission.include?(45)
      @posts = Post.includes([:type_comment]).where("transformer_id = ?",params[:transformer_id])
      @physical_posts = PhysicalPost.includes([:type_comment]).where("transformer_id = ?",params[:transformer_id])
    else
