class ComparisonManagement::GasesController < ApplicationController
  before_action :authenticate_user


  def show
    if user_permission.include?(153)
      redirect_back(fallback_location: step1_comparison_management_gases_url)
    else
  ...
  # GET/POST /comparison_management/gases
  def step2
    if user_permission.include?(153)
      if params[:transformer_ids].nil?
        ...
        @list_chromatographicals = Chromatographical.where("deleted= 0 AND transformer_id IN (?)",@search_tramsformers)        
      end
    else
      flash[:notice] = "No tienes acceso."
      flash[:type_message] = "red"
