class CustomerManagement::UserCustomerAreasController < ApplicationController
  ...
  def index
      ...
      @areas = CustomerArea.where("deleted= 0 AND customer_id IN (?)",@user_customers.map { |e| e.customer_id }).order("name ASC")
